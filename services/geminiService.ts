...
export async function analyzeCharacterFeatures(imageDataUrl: string): Promise<string> {
    try {
 ...
 */
function getFallbackPrompt(decade: string, characterDescription: string = ""): string {
 ...
 */
export async function generateDecadeImage(imageDataUrl: string, prompt: string, characterDescription: string = ""): Promise<string> {
  const match = imageDataUrl.match(/^data:(image\/\w+);base64,(.*)$/);
  if (!match) {
